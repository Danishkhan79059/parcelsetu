  ...
  const useCases = [
    {
      ...
      points: ['Same-day & next-day dispatch', 'Full-truckload & part-truckload options', 'Route planning & milk runs'],
      color: '#1F4E8C',
    },
    {
      icon: '🏪',
      ...
      points: ['Fixed routes & dedicated vehicles', 'Shipment visibility for your partners', 'POD and reconciliation support'],
      color: '#FF6B2B',
    },
    {
      icon: '🔒',
      ...
      points: ['Secured vehicles & monitored routes', 'Insurance & documentation support', 'Dedicated coordination team'],
      color: '#A855F7',
    },
  ]

  const whyChoose = [
    { title: 'High On-Time Performance',    desc: 'Optimised routing, disciplined line-haul planning and strong partner network to achieve high on-time delivery performance for B2B shipments.', bg: 'linear-gradient(135deg,#0F1C35,#1F4E8C)' },
    { title: 'End-to-End Visibility',       desc: 'Track every movement from pickup to delivery, with status updates, events and POD information centralised for your operations team.',             bg: 'linear-gradient(135deg,#FF6B2B,#e55a1c)' },
    { title: 'Process-Driven Operations',   desc: 'Standard operating procedures, checklists and escalation matrices to reduce damage, pilferage and operational leakages.',                       bg: 'linear-gradient(135deg,#1a3a6e,#2563EB)' },
  ]

  const onboardSteps = [
    { step:'01', title:'Requirement Study',    text:'We understand your lanes, volumes, service levels and current pain points.',                        color:'#1F4E8C' },
    { step:'02', title:'Network Design',       text:'We propose an optimised network, routes, vehicles and operating model.',                            color:'#FF6B2B' },
    { step:'03', title:'Pilot & Stabilisation',text:'We run pilots on key lanes, fine-tune operations and standardise processes.',                       color:'#A855F7' },
    { step:'04', title:'Scale Up',             text:'We expand coverage, add lanes and continuously improve performance.',                               color:'#10B981' },
  ]
    ...
    <div style={{ minHeight: '100vh', fontFamily: "'Outfit','Segoe UI',sans-serif", background: '#F8FAFC' }}>
      ...
      <section className="hero-bg" style={{ padding: '90px 0 80px' }}>
        ...
        <div className="hero-gif-wrap">
          <Image src="/images/b2b.gif" alt="" fill style={{ objectFit: 'cover' }} unoptimized />
        </div>
        ...
        <div style={{ position:'relative',zIndex:1,padding:'0 24px',maxWidth:860,margin:'0 auto' }}>
          <div className="badge-pill" style={{ marginBottom:20 }}>🚛 B2B & Critical Logistics</div>
          ...
          <p style={{ fontSize:'clamp(14px,2vw,18px)',color:'rgba(255,255,255,.74)',lineHeight:1.8,marginBottom:36,maxWidth:600 }}>
          ...
          <div style={{ display:'flex',gap:14,flexWrap:'wrap' }}>
            <Link href="/contactus" className="btn-accent">Talk to Logistics Expert</Link>
          ...
          <div className="two-col">
            ...
            <div>
              ...
              <p style={{ fontSize:15,color:'#6B7280',lineHeight:1.9,marginBottom:14 }}>
                B2B &amp; critical logistics focuses on time-bound, high-value and
                operationally sensitive shipments between manufacturers, regional
                distribution centres, wholesalers, and large retail networks.
              </p>
              <p style={{ fontSize:15,color:'#6B7280',lineHeight:1.9,marginBottom:14 }}>
                At ParcelSetu, we design dedicated movement plans, secure routing and
                optimized consolidation so that your production lines never stop
                due to missing parts and your distributors always receive stock on time.
              </p>
              <p style={{ fontSize:15,color:'#6B7280',lineHeight:1.9 }}>
                From plant dispatch to last-mile delivery at DCs, our network and
                technology ensure high on-time delivery rate, real-time tracking,
            ...
            <div style={{ position:'relative',height:400,borderRadius:24,overflow:'hidden',boxShadow:'0 20px 56px rgba(0,0,0,.1)',border:'1.5px solid #F3F4F6' }}>
              <Image src="/images/ship.png" alt="B2B logistics operations" fill style={{ objectFit:'cover' }} />
      ...
      <section style={{ padding:'72px 24px',background:'#F8FAFC' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ display:'flex',alignItems:'center',gap:16,marginBottom:44 }}>
            <div className="accent-bar" />
            <div>
              <div className="badge-pill" style={{ marginBottom:8 }}>🗺️ Use Cases</div>
          ...
          <div className="three-col">
            {useCases.map((uc,i) => (
              <div key={i} className="uc-card" style={{ '--uc-color':uc.color }}>
                <style>{`.uc-card:nth-child(${i+1})::before { background: linear-gradient(90deg,${uc.color},${uc.color}88); }`}</style>
                <div className="uc-icon" style={{ background:`rgba(${uc.color==='#1F4E8C'?'31,78,140':uc.color==='#FF6B2B'?'255,107,43':'168,85,247'},.08)` }}>
                  {uc.icon}
                </div>
                <h3 style={{ fontSize:17,fontWeight:800,color:'#111827',marginBottom:10 }}>{uc.title}</h3>
                <p style={{ fontSize:13,color:'#6B7280',lineHeight:1.8,marginBottom:16 }}>{uc.desc}</p>
                <ul style={{ listStyle:'none',padding:0,display:'flex',flexDirection:'column',gap:8 }}>
                  {uc.points.map((p,j) => (
                    <li key={j} style={{ display:'flex',alignItems:'center',gap:8,fontSize:13,color:'#374151' }}>
                      <span style={{ color:uc.color,fontWeight:900,fontSize:14,flexShrink:0 }}>✓</span>{p}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="three-col">
            {whyChoose.map((wc,i) => (
              <div key={i} className="wc-card" style={{ background:wc.bg }}>
                ...
                <h3 style={{ fontSize:20,fontWeight:900,color:'white',marginBottom:14 }}>{wc.title}</h3>
                <p style={{ fontSize:14,color:'rgba(255,255,255,.76)',lineHeight:1.8 }}>{wc.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:'72px 24px',background:'linear-gradient(180deg,#F1F5F9 0%,#F8FAFC 100%)' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:52 }}>
            <div className="badge-pill" style={{ margin:'0 auto 16px' }}>🚀 Onboarding</div>
            <h2 style={{ fontSize:'clamp(22px,4vw,38px)',fontWeight:900,color:'#111827',marginBottom:12 }}>
              How We Onboard B2B Clients
            </h2>
            <p style={{ fontSize:15,color:'#6B7280',maxWidth:520,margin:'0 auto',lineHeight:1.7 }}>
              A structured engagement model that makes it simple to start, scale and
              optimise your B2B logistics with us.
            </p>
            <div className="sdivider" />
          </div>
            ...
            <div className="four-col">
              {onboardSteps.map((step,i) => (
                <div key={i} className="ob-card">
                  <div className="ob-num" style={{ background:`linear-gradient(135deg,${step.color},${step.color}cc)` }}>
                    {step.step}
                  </div>
        ...
        <div style={{ maxWidth:820,margin:'0 auto' }}>
          <div className="cta-wrap" style={{ padding:'56px 44px',textAlign:'center' }}>
            ...
            <div style={{ position:'relative',zIndex:1 }}>
              <div className="badge-pill" style={{ margin:'0 auto 18px' }}>📞 Get in Touch</div>
              ...
              <p style={{ fontSize:15,color:'rgba(255,255,255,.7)',marginBottom:32,maxWidth:460,margin:'0 auto 32px',lineHeight:1.7 }}>
                Talk to our logistics experts and get a customised B2B solution for your business.
              </p>
              <div style={{ display:'flex',gap:16,justifyContent:'center',flexWrap:'wrap' }}>
                <Link href="/contactus" className="btn-accent" style={{ fontSize:15,padding:'14px 32px' }}>Talk to Expert →</Link>
